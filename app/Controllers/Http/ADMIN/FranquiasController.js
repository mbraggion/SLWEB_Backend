...
class FranquiasController {
  ...
  async Store({ request, response }) {
    ...
    try {
      ...
      await Database.raw("insert into dbo.Receita select ?, RecId, RecDesc, RecStatus from dbo.Receita where GrpVen = '000000'", [String(FormData.GrpVen).trim().substring(0, 6)])
      await Database.raw("insert into dbo.ReceitaDet select ?, RecId, GprdId, RdetQtd, RDetH2O from dbo.ReceitaDet where GrpVen = '000000'", [String(FormData.GrpVen).trim().substring(0, 6)])
